...
import { AlignVerticalJustifyCenter, CheckCircle, Download } from 'lucide-react'
...
import { FREE_PDF_SIZE_LIMIT } from '../utils/uploadLimits'
...
const AutoDeskewPage = () => {
  ...
  const freeLimitLabel = formatFileSize(FREE_PDF_SIZE_LIMIT)
  ...
  const onDropRejected = (fileRejections) => {
    fileRejections.forEach(({ file, errors }) => {
      if (errors[0]?.code === 'file-too-large' && freeLimitActive) {
        toast.error(`${file.name} exceeds the ${freeLimitLabel} free limit. Upgrade to upload larger PDFs.`)
        return
      }
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    multiple: true,
    maxSize: freeLimitActive ? FREE_PDF_SIZE_LIMIT : undefined
  })
  ...
  const handleProcess = async () => {
    ...
    if (freeLimitActive && files.some((file) => file.size > FREE_PDF_SIZE_LIMIT)) {
      toast.error(`Free users can upload PDFs up to ${freeLimitLabel}. Please remove oversized files.`)
      return
    }
    ...
    try {
      const resultBlobs = await pdfService.autoDeskewPdf(files, (progressValue) => {
      ...
      toast.success(`${resultBlobs.length} PDF${resultBlobs.length > 1 ? 's' : ''} deskewed successfully!`)
    } catch (error) {
      toast.error(error.message || 'Failed to deskew PDF')
    } finally {
      setProcessing(false)
    }
  }
  ...
  const handleDownload = () => {
    if (results.length > 0) {
      results.forEach((result, index) => {
        const originalFileName = files[index]?.name || `file_${index + 1}.pdf`
        const fileName = originalFileName.replace('.pdf', '_deskewed.pdf')
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <div className="inline-flex items-center justify-center w-16 h-16 bg-purple-100 rounded-full mb-4">
              <AlignVerticalJustifyCenter className="w-8 h-8 text-purple-600" />
            </div>
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Auto Deskew PDFs</h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Automatically detect skewed pages in pdf and correct them with one click. 
            </p>
          </motion.div>
          ...
          >
            <div className="p-8">
              ...
              >
                <input {...getInputProps()} />
                <AlignVerticalJustifyCenter className="w-12 h-12 text-purple-500 mx-auto mb-4" />
                <p className="text-lg font-medium text-gray-700 mb-2">
                  {files.length > 0 ? `${files.length} PDF${files.length > 1 ? 's' : ''} selected` : 'Drop your PDFs here'}
                </p>
                <p className="text-sm text-gray-500">
                  {files.length > 0 ? 'Click to change files' : 'or click to browse (multiple files supported)'}
                </p>
                {freeLimitActive && (
                  <p className="mt-3 text-sm text-amber-600">
                    Free users can upload PDFs up to {freeLimitLabel}.
                  </p>
                )}
                {files.length > 0 && (
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-green-900 mb-2">
                    Successfully Deskewed {results.length} PDF{results.length > 1 ? 's' : ''}!
                  </h3>
          ...
          >
            {!processing && results.length === 0 && (
              <button
                ...
                className="px-8 py-4 bg-purple-600 text-white rounded-xl font-semibold hover:bg-purple-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors shadow-lg hover:shadow-xl"
              >
                {`Auto Deskew ${files.length > 0 ? files.length : ''} PDFs${files.length > 1 ? 's' : ''}`}
              </button>
            )}
            ...
            {processing && (
              <button
                disabled
                className="px-8 py-4 bg-purple-600 text-white rounded-xl font-semibold opacity-75 cursor-not-allowed shadow-lg"
              >
                Processing... {progress}%
              </button>
            )}
            ...
            {!processing && results.length > 0 && (
              <>
                ...
                >
                  ...
                  <span>Download {results.length > 1 ? `All ${results.length} ` : ''}Deskewed PDF{results.length > 1 ? 's' : ''}</span>
                </button>
              </>
            )}
          </motion.div>
