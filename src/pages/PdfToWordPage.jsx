...
import { CheckCircle, Download, FileType } from 'lucide-react'
...
const PdfToWordPage = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const resultBlobs = await pdfService.pdfToWord(files, (progressValue) => {
      ...
      toast.success(`${resultBlobs.length} PDF${resultBlobs.length > 1 ? 's' : ''} converted to Word successfully!`)
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Convert your PDF files to editable Word documents with one click. 
            </p>
          </motion.div>
          ...
          >
            <div className="p-8">
              ...
              >
                <input {...getInputProps()} />
                <FileType className="w-12 h-12 text-purple-500 mx-auto mb-4" />
                  ...
                  <div className="mt-4 text-left max-h-40 overflow-y-auto">
                    {files.map((file, index) => (
                      <div key={index} className="text-sm text-gray-600 py-1 flex items-center space-x-2">
                        <FileType className="w-4 h-4 text-purple-500 flex-shrink-0" />
              ...
              <div className="space-y-2 text-sm">
                {files.map((file, index) => (
                  <div key={index} className="flex items-center justify-between text-gray-600">
                    ...
                    <span className="ml-2 text-purple-600 flex-shrink-0">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-green-900 mb-2">
                    Successfully Converted {results.length} PDF{results.length > 1 ? 's' : ''} to Word!
                  </h3>
          ...
          >
            {!processing && results.length === 0 && (
              ...
              >
                                {`Convert to Word ${files.length > 0 ? `(${files.length} file${files.length > 1 ? 's' : ''})` : ''}`}
              </button>
            )}
            ...
            {!processing && results.length > 0 && (
              <>
                ...
                >
                  ...
                  <span>Download {results.length > 1 ? `All ${results.length} ` : ''}Word Document{results.length > 1 ? 's' : ''}</span>
                </button>
              </>
            )}
          </motion.div>
