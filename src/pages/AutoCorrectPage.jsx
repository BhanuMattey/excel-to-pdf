...
import { ScanLine, CheckCircle, Download } from 'lucide-react'
...
const FREE_PDF_SIZE_LIMIT = 5 * 1024 * 1024
...
const AutoCorrectPage = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      const resultBlobs = await pdfService.autoCorrectPdf(files, (progressValue) => {
      ...
      toast.success(`${resultBlobs.length} PDF${resultBlobs.length > 1 ? 's' : ''} corrected successfully!`)
    } catch (error) {
      toast.error(error.message || 'Failed to correct PDF')
    } finally {
      setProcessing(false)
    }
  }
  ...
  const handleDownload = () => {
    if (results.length > 0) {
      results.forEach((result, index) => {
        ...
        const fileName = originalFileName.replace('.pdf', '_corrected.pdf')
      ...
      <main className="flex-grow pt-24 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-gray-900 mb-4">Auto Correct PDF</h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Automatically detect rotated and flipped pages in pdf and correct them with one click. 
            </p>
          </motion.div>
          ...
          >
            <div className="p-8">
              ...
              >
                <input {...getInputProps()} />
                <ScanLine className="w-12 h-12 text-purple-500 mx-auto mb-4" />
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-green-900 mb-2">
                    Successfully Corrected {results.length} PDF{results.length > 1 ? 's' : ''}!
                  </h3>
          ...
          >
            {!processing && results.length === 0 && (
              ...
              >
                {`Auto Correct ${files.length > 0 ? files.length : ''} PDF${files.length > 1 ? 's' : ''}`}
              </button>
            )}
            ...
            {!processing && results.length > 0 && (
              <>
                ...
                >
                  ...
                  <span>Download {results.length > 1 ? `All ${results.length} ` : ''}Corrected PDF{results.length > 1 ? 's' : ''}</span>
                </button>
              </>
            )}
          </motion.div>
